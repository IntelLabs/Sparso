...
function connect_finish(state)
    connect(state.cur_bb,state.basic_blocks[-2])
end
    ...
    for i in bbs
      ...
      elseif length(bb.preds) == 1 && length(bb.succs) == 1
        ...
        if length(pred.succs) == 1
            succ = first(bb.succs)
            delete!(pred.succs, bb)
            delete!(succ.preds, bb)
            push!(pred.succs, succ)
...
function from_label(label, state, callback, cbdata)
    ...
    connect(state.cur_bb, state.basic_blocks[label])
...
function from_if(args, depth, state, callback, cbdata)
    ...
    connect(state.cur_bb, state.basic_blocks[implied])
    connect(state.cur_bb, state.basic_blocks[else_label])
...
function from_expr(ast::Any, depth, state, top_level, callback, cbdata)
  ...
  elseif asttyp == Expr
    ...
    elseif head == :copy 
        ...
        # skip
    elseif head == :stencil!
        #from_exprs(args, depth+1, state)
        local sbufs = args[3]
        for i = 1:length(sbufs)
            from_expr(sbufs[i], depth, state, false, callback, cbdata)
        end
    elseif head == :copyast
