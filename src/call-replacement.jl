...
@doc """
    log1p(x) ==> SparseAccelerator.log1p!(temp, x), where temp is a temporary to be
    generated by the compiler.
"""
...
#    println("\tBottom up:")
        ...
        # Now do splittable patterns. They should be applied bottom up, because
...
function replace_calls(
    ...
    call_sites  = CallSites(Set{CallSite}(), FunctionRegion(func_ast), lambda, symbol_info, 
