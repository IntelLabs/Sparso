...
function dss_analyze(A::SparseMatrixCSC)
  ...
  error = ccall((:dss_create, SparseAccelerator.LIB_PATH), Cint,
        (Ptr{Void}, Ptr{Cint}),
        handle, &opt)
  ...
  m = size(A, 1)
  nnz = A.colptr[m + 1] - 1
  error = ccall((:dss_define_structure, SparseAccelerator.LIB_PATH), Cint,
  ...
  error = ccall((:dss_reorder, SparseAccelerator.LIB_PATH), Cint,
...
function dss_factor(handle, A::SparseMatrixCSC)
  opt = MKL_DSS_DEFAULTS
  error = ccall((:dss_factor_real, SparseAccelerator.LIB_PATH), Cint,
